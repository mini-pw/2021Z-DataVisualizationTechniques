...
data <- read_excel(paste(path,"2-4_achievement-trends-S4",fileExtension,sep=""),sheet=2)
data
end<- which(apply(data[1], 1, function(r) r == c("Benchmarking Participants")))
end
df <- data %>%
  select(c(1,3,19,20,5,23,24)) %>%
  slice(7:end-1)
